...
//
// if youre writing a argument handler
// feel free to change these strings and 
// use the plib_Error macro to get the 
...
// If an argument fails to parse within 
// the parse function this macro reverts
// changes to the argv index so that error
// codes show relivant argument information.
// This function is suppost to be called 
// in place of return, this is where 'e' 
// comes in. _plib_strlen () is just a 
// custom strlen without <string.h>.

// Assume v already has a null terminator
	...
	for (out->index = 1; out->index < c; ++out->index)
	  {
		...
		const int arg_takes_val   = plib_ReadProperty(ar[arg_s].opt, PLIB_TAKESVALUE);
...
static int plib_CheckRequired (struct plib_Argument *ar)
{
	...
	// it will return the index of the required argument instead 
	// of the index of the last parsed argument. 
	//
	// Look at https://github.com/x3hy/plate and open arg.c for a
...
#define plib_ErrorCause(ar) \
	(PL_RETURN.code == PL_NO_REQUIRED_ARG) \
		? ar[PL_RETURN.index].flag \
